...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext, nDepth )
	{
		...
		{
			$ImgCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Трансляция') );
		}
		...
		if( rgRecommendationReasons.recommended_by_friend )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('friend').html( "<strong>\u0420\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u043e\u0432\u0430\u043d\u043e<\/strong> \u0432\u0430\u0448\u0438\u043c \u0434\u0440\u0443\u0433\u043e\u043c, <span>%1$s<\/span>".replace("%1$s", friend.name ) );
		...
		else if( rgRecommendationReasons.recent_release_by_creator )
		{
			...
			var $ReasonLocal = creator.link.indexOf( 'developer/' ) >= 0 ? "<strong>\u0420\u0430\u0437\u0440\u0430\u0431\u043e\u0442\u0447\u0438\u043a:<\/strong><br><span>%1$s<\/span>" : "<strong>\u0418\u0437\u0434\u0430\u0442\u0435\u043b\u044c:<\/strong><br><span>%1$s<\/span>";
		...
		else if( rgRecommendationReasons.recommended_by_curator )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('curator').html( "<strong>\u0420\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u043e\u0432\u0430\u043d\u043e:<\/strong> <br><span>%1$s<\/span>".replace("%1$s", V_EscapeHTML( curator.name ) ) );
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>\u0420\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u043e\u0432\u0430\u043d\u043e,<\/strong> \u043f\u043e\u0442\u043e\u043c\u0443 \u0447\u0442\u043e \u0432\u0430\u0441 \u0438\u043d\u0442\u0435\u0440\u0435\u0441\u0443\u044e\u0442 \u043f\u043e\u0445\u043e\u0436\u0438\u0435 \u0438\u0433\u0440\u044b" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong><\/strong>\u0412\u044b \u0438\u0433\u0440\u0430\u043b\u0438 \u0432 \u0438\u0433\u0440\u044b \u0441 \u043c\u0435\u0442\u043a\u0430\u043c\u0438:" );
		...
		else
		{
			...
			{
				strStatus = 'Ранний доступ уже открыт';
			}
		...
		if( bShowAdditionalReasons )
		{
			...
			for( var key in rgRecommendationReasons )
			{
				...
				if( key == 'top_seller' )
					strReason = "\u041b\u0438\u0434\u0435\u0440 \u043f\u0440\u043e\u0434\u0430\u0436";
				else if( key == 'new_release' )
					strReason = "\u041d\u043e\u0432\u0438\u043d\u043a\u0430";
				else if( key == 'recommended')
					strReason = "\u0421\u0445\u043e\u0436\u0435 \u0441 \u0442\u0435\u043c, \u0432\u043e \u0447\u0442\u043e \u0432\u044b \u0438\u0433\u0440\u0430\u0435\u0442\u0435";
				else
					continue;
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							$elReviewData.append( $J('<div>', {'class': 'title'}).text( rgData.appids ? 'Отзывы пользователей о продуктах в этом наборе:' : 'Все обзоры:') );
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/ru/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
						...
						for( var i=0; i < 4; i++ )
						{
							...
							{
								var $elScreenshot = $J('<div>', {'class': 'screenshot empty'}).append("<div>").text("\u042d\u0442\u043e\u0442 \u0441\u043a\u0440\u0438\u043d\u0448\u043e\u0442 \u043c\u043e\u0436\u0435\u0442 \u0441\u043e\u0434\u0435\u0440\u0436\u0430\u0442\u044c \u043c\u0430\u0442\u0435\u0440\u0438\u0430\u043b\u044b \u0434\u043b\u044f \u0432\u0437\u0440\u043e\u0441\u043b\u044b\u0445.");
								$elInfoDiv.append($elScreenshot);
							}
						}
	...
	RenderRecommendedByDeepDiveCarousel: function()
	{
		...
		focusedAppTitle = 'Поскольку вы играли в <strong>%s</strong>'.replace( "%s", focusedAppTitle);
	...
	RenderCommunityRecommendations: function()
	{
		...
		{
			elemPrefLastUpdated.text( 'Текущие настройки сохранены %1$s'.replace( "%1$s", GHomepage.strCommunityRecommendationsPrefLastSaved ) );
		}
	...
	BuildHomePageGenericCap: function( strFeatureContext, unAppID, unPackageID, unBundleID, rgOptions, nDepth )
	{
		...
		{
			$CapCtn.append(
					$J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Трансляция')
			);
		}
	...
	BuildCommunityRecommendationCap: function( strFeatureContext, oItem )
	{
		...
		if ( numReviews > 1 )
		{
			...
			let strNumReviews = 'Обзоров: %1$s из %2$s'.replace( "%1$s", 1 ).replace( "%2$s", numReviews );
			...
			arrowLeft.click( function() {
				...
				var strNumReviews = 'Обзоров: %1$s из %2$s'.replace( "%1$s", curIdx + 1 ).replace( "%2$s", numReviews );
		...
		for( var i = 0; i < rgReviews.length; ++i)
		{
			...
			var $ViewMore = $J( '<div class="view_more"><a href="' + review.review_link + '" target="_blank">Читать дальше</a></div>' );
			...
			if ( reviewer )
			{
				...
				{
					var strHours = 'Сыграно %s ч. в момент написания'.replace( "%s", v_numberformat( review.playtime_at_review / 60, 1 ) );
					$AuthorDetails.append( $J( '<div>', { class: "hours ellipsis", text: strHours } ) );
				}
				if ( review.votes_up > 0 )
				{
					var voteUpText = '1 пользователь посчитал этот обзор полезным';
					if ( review.votes_up > 1 )
					{
						voteUpText = 'Пользователей, посчитавших обзор полезным: %2$s'.replace( "%2$s", v_numberformat( review.votes_up ) );
					}
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/ru/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
	...
	RenderMarketingMessages: function(  )
	{
		...
		var createMessageElementFunc = function( message, cssClass )
		{
			...
			if ( rgItemData )
			{
				if( rgItemData.has_live_broadcast )
				{
					$MessageCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Трансляция') );
				}
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	this.m_$Popup.append( $J('<div/>', {'class': 'home_viewsettings_instructions' } ).text( 'Выберите типы товаров, которые вы хотите здесь видеть' ) );
	...
	if ( this.m_ApplicableSettings.recommended_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_for_you', 'Рекомендовано вам' ) );
	...
	if ( this.m_ApplicableSettings.dlc_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc_for_you', 'Доп. контент для ваших игр и программ' ) );
	...
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Новинки' ) );
	if ( this.m_ApplicableSettings.dlc )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc', 'Дополнительный контент' ) );
	...
	if ( this.m_ApplicableSettings.localized )
		this.m_$Popup.append( this.RenderCheckbox( 'localized', 'Игры на моем языке' ) );
	if ( this.m_ApplicableSettings.virtual_reality )
		this.m_$Popup.append( this.RenderCheckbox( 'virtual_reality', 'Виртуальная реальность' ) );
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', 'Рекомендовано вашими кураторами' ) );

	if ( this.m_ApplicableSettings.only_current_platform )
	{
		// this one is a little magic
		if ( GDynamicStore.s_bUserOnLinux )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Доступно для Linux/SteamOS' ) );
		else if ( GDynamicStore.s_bUserOnMacOS )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Доступно для macOS' ) );
	}
...
{
	return 'https://cdn.akamai.steamstatic.com/steamcommunity/public/images/avatars/' + strAvatarHash.substr( 0 , 2 ) + '/' + strAvatarHash + ( sizeStr || '' ) + '.jpg';
}
...
GSteamCurators = {
    ...
    BuildResponsiveHomePageGiantCap: function( strFeatureContext, oItem )
	{
        ...
        var $Status = $J('<div/>', {'class': 'recommendation_status recommended'}).text( "\u0420\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u0430\u0446\u0438\u0438" );
