...
GSteamBroadcasts = {
	...
	CreateBroadcastCapsule: function( oItem, bAddTitleBar )
	{
		...
		var rgPlayIconProperties = { src: 'https://store.akamai.steamstatic.com/public/shared/images/apphubs/play_icon80.png', class: "live_stream_play_icon" };
			...
			if ( data.success == 1 && data.filtered.length > 0 )
			{
				for( var i = 0; i < data.filtered.length; i++ )
				{
					...
					if ( rgAppInfo )
					{
						...
						{
							$Icon = $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Трансляция');
							$J(this).append( $Icon );
						});
					}
				}
...
function BuildCreatorCapsuleToAppend( oItem )
{
	...
	$FollowLink.bind('updateFollowState', function () {
		...
		{
			$FollowLink.html( '<span><img src="https://store.akamai.steamstatic.com/public/images/v6/ico/ico_selected_green.png">В подписках</span>' );
